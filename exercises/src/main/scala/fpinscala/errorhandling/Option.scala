...
object Option {
  def failingFn(i: Int): Int = {
    val y: Int = throw new Exception("fail!") // `val y: Int = ...` declares `y` as having type `Int`,
    ...
    // A `catch` block is just a pattern matching block like the ones we've seen. `case e: Exception`
    // is a pattern that matches any `Exception`, and it binds this value to the identifier `e`.
  ...
  def failingFn2(i: Int): Int = {
    try {
      ...
      x + ((throw new Exception("fail!")): Int) // A thrown Exception can be given any type;
                                                // here we're annotating it with the type `Int`
    }
    catch {
      case e: Exception => 43
    }
